...
source "vmware-iso" "debian" {
  iso_url           = "https://cdimage.debian.org/debian-cd/12.8.0/amd64/iso-cd/debian-12.8.0-amd64-netinst.iso"
...
#   ]
  ...
  boot_command = [
    ...
    "linux /install.amd/vmlinuz",            # Load the AMD-compatible kernel
    " auto-install/enable=true",             # Enable automatic installation
    " debconf/priority=critical",            # Set the installation to critical priority
    " netcfg/hostname=debian-12",            # Set the hostname
    " netcfg/get_domain=",                   # Leave the domain empty
    " preseed/url=http://{{ .HTTPIP }}:{{ .HTTPPort }}/preseed.cfg --- quiet", # Provide the preseed file
    "<enter>",                               # Submit the command
    "initrd /install.amd/initrd.gz",         # Load the AMD-compatible initial RAM disk
    "<enter>",                               # Submit the command
